...
def _select_initial_step(fun, t0, y0, xa, order, rtol, atol, f0=None):
    ...
    if len(y0)==1:
        scale = tuple(atol_ + torch.abs(y0_[:,:1]) * rtol_ for y0_, atol_, rtol_ in zip(y0, atol, rtol))
        d0 = tuple(_norm(y0_[:, :1] / scale_) for y0_, scale_ in zip(y0, scale))
    else:
        scale0 = (atol[0] + torch.abs(y0[0][:, :1]) * rtol[0],)
        scale1 = (atol[1] + torch.abs(y0[1][:, :1]) * rtol[1],)
        scale2 = (atol[2] + torch.abs(y0[2]) * rtol[2],)
        scale3 = (atol[3] + torch.abs(y0[3]) * rtol[3],)
        scale = scale0 + scale1 + scale2 + scale3
        del scale0, scale1, scale2, scale3

        d0_0 = (_norm(y0[0][:, :1] / scale[0]),)
        d0_1 = (_norm(y0[1][:, :1] / scale[1]),)
        d0_2 = (_norm(y0[2] / scale[2]),)
        d0_3 = (_norm(y0[3] / scale[3]),)
        d0 = d0_0 + d0_1 + d0_2 + d0_3
        del d0_0, d0_1, d0_2, d0_3
    ...
    if len(y0)==1:
        y1 = tuple(y0_[:,:1] + h0 * f0_ for y0_, f0_ in zip(y0, f0))
        #y1 = tuple(y0_[:, :1] + torch.unsqueeze(h0 * f0_,1) for y0_, f0_ in zip(y0, f0))
        y1 = (torch.cat((y1[0],xa[0,:,:]),1),)
        f1 = fun(t0 + h0, y1)
    else:
        y1_0 = y0[0][:, :1] + h0 * f0[0]
        y1_0 = (torch.cat((y1_0, xa),1),)

        y1_1 = (y0[1][:, :1] + h0 * f0[1],)
        y1_2 = (y0[2] + h0 * f0[2],)
        y1_3 = (y0[3] + h0 * f0[3],)

        y1 = y1_0 + y1_1 + y1_2 + y1_3
        del y1_0, y1_1, y1_2, y1_3

        f1 = fun(t0 + h0, y1)
        f1 = (f1[0],) + (f1[1][:,:1],) + (f1[2],) + (f1[3],)
    ...
    if error_tol is None:
        ...
        if len(y0)==1:
            error_tol = tuple(
                ...
                for atol_, rtol_, y0_, y1_ in zip(atol, rtol, y0, y1)  # y0 has been changed
            )
        else:
            c_flag = 1
            for atol_, rtol_, y0_, y1_ in zip(atol, rtol, y0, y1):
                if c_flag == 1: # only removing the first element features
                    error_tol = [atol_ + rtol_ * torch.max(torch.abs(y0_[:, :1]), torch.abs(y1_))]
                    c_flag = c_flag + 1
                else:
                    error_tol.append(atol_ + rtol_ * torch.max(torch.abs(y0_), torch.abs(y1_)))
                    c_flag = c_flag + 1
            error_tol = tuple(error_tol)
        #########################################################################

        #error_tol = tuple(
        #    atol_ + rtol_ * torch.max(torch.abs(y0_[:,:1]), torch.abs(y1_))
        #    for atol_, rtol_, y0_, y1_ in zip(atol, rtol, y0, y1) # y0 has been changed
        #)
