...
if __name__ == '__main__':
    ...
    #### model
    #rec = RecognitionRNN1(inp_dim, latent_dim, batch_size).to(device)
    #rec = LSTMClassifier(inp_dim, latent_dim, batch_size, ugpu).to(device)
    ...
    for itr in range(args.nepochs+1): # epoch number
        ...
        with torch.no_grad(): ### after each epoch training
            ...
            if ccc_test[itr] > ccc_best[0]:
                ...
                io.savemat(savepath + 'p_tsnew_' + str(args.scale) + '_' + '_' + str(seq) + name[
                    dim] + '.mat',
    ...
    io.savemat( savepath + 'CCC_testsnew_' + str(args.scale) + '_'  + '_' + str(seq) + name[dim] + '.mat',
               {"ccc_test": ccc_test})
    io.savemat( savepath + 'CCC_trainsnew_' + str(args.scale) + '_'  + '_' + str(seq) + name[dim] + '.mat',
               {"ccc_train": ccc_train})
