...
parser.add_argument('--filwin', type=int, default=100)# 4s averaging filter
...
class LatentODEfunc(nn.Module):
    ...
    def forward(self, t, x):
        ...
        #out = (torch.tanh(out) + 1.0) / 2.0 * 2.5 - 0.9583  # mapping output y to the range of y in the training dataset
        return out
...
def postprocessing(x, ref, mod): # post processing predictions
    mu = torch.mean(ref)
    std = torch.std(ref)
    if mod == 0: #only conducting shifting
        x = x - torch.mean(x) + mu
    else:# shifting and scaling
...
name = [str(nhidden) + 'ar',str(nhidden) + 'vr']
...
if __name__ == '__main__':
    ...
    params = (list(rec.parameters()) + list(func.parameters()))
    optimizer = optim.Adam(params, lr=args.lr)
    scheduler = optim.lr_scheduler.ExponentialLR(optimizer=optimizer, gamma=args.lr_decay)
    loss_meter = RunningAverageMeter()

    ########################## mean averaging after preditions ###############################
    mean_conv = nn.Conv1d(in_channels=1,
                          out_channels=1,
                          kernel_size=args.filwin,
                          padding=int(args.filwin/2))
    kernel_weights = np.ones(args.filwin) / args.filwin
    mean_conv.weight.data = torch.FloatTensor(kernel_weights).view(1, 1, args.filwin)
    ...
    for itr in range(args.nepochs+1): # epoch number
        for i,data in enumerate(train_loader):
            ...
            #################################################################
            if i == 0:
                p_tr = pred_train[:batch_size]
            else:
                p_tr = torch.cat((p_tr, pred_train[:batch_size]))

        ccc_train[itr] = cc_cal(p_tr, Y_train.to(device))
        ...
        with torch.no_grad(): ### after each epoch training
            ...
            for i, data in enumerate(test_loader):# for each utterence
                ...
                te_lab1 = chunking_frame(torch.unsqueeze(te_lab, 1).cpu().detach().numpy(), ts_size, 0)
                ...
                p_test = odeint(func, te_lab1[delay,:,:], tt, te_fea, args.rtol, args.atol)
            ...
            if ccc_test[itr] > ccc_best[0]:
                ...
                p_tr1 = postprocessing(p_tr, Y_train.to(device), 0)
                p_tr2 = postprocessing(p_tr, Y_train.to(device), 1)
                maxind = np.argmax([cc_cal(p_tr, Y_train.to(device)), cc_cal(p_tr1, Y_train.to(device)),
                                    cc_cal(p_tr2, Y_train.to(device))])

                p_ts = mean_conv(p_ts.unsqueeze(0).unsqueeze(0))[0, 0, :-1]
                if maxind == 0:
                    ccc_test[itr] = cc_cal(p_ts, Y_test.to(device))
                elif maxind == 1:
                    p_ts1 = postprocessing(p_ts, Y_train.to(device), 0)
                    ccc_test[itr] = cc_cal(p_ts1, Y_test.to(device))
                else:
                    p_ts2 = postprocessing(p_ts, Y_train.to(device), 1)
                    ccc_test[itr] = cc_cal(p_ts2, Y_test.to(device))

                print('CCC best = ' + str(ccc_best[0]) + ' epoch = ' + str(itr))
                io.savemat(savepath + 'p_ts_' + '_' + str(seq) + name[dim] + features[ftype] + '.mat',
    ...
    io.savemat( savepath + 'CCC_test_' + '_' + str(seq) + name[dim] + features[ftype] + '.mat',
               {"ccc_test": ccc_test})
    io.savemat(savepath + 'CCC_train_' + '_' + str(seq) + name[dim] + features[ftype] + '.mat',
              {"ccc_train": ccc_train})
